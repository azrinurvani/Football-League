...
class DetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val item  =intent.getParcelableExtra<LeagueItem>(MainActivity.PARCELABLE_ITEM_LEAGUE)
        DetailActivityUI(item).setContentView(this)

        supportActionBar?.setDisplayHomeAsUpEnabled(true)
    }

    class DetailActivityUI(val item:LeagueItem): AnkoComponent<DetailActivity>{
        val id_view =1
        val id_image =2
        val id_name = 3
